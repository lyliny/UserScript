...
// @name         搜索引擎切换器魔改版
// @version      1.4
// @description  快速切换搜索引擎。拥有高斯模糊外观、深色模式适配、侧栏自动收缩。原作者(https://greasyfork.org/zh-CN/scripts/490643)
// @author       Gemini修复版
...
// @downloadURL  https://raw.githubusercontent.com/lyliny/UserScript/main/Search%20Engine%20Switcher/Search-Engine-Switcher.user.js
// @updateURL    https://raw.githubusercontent.com/lyliny/UserScript/main/Search%20Engine%20Switcher/Search-Engine-Switcher.user.js
// ==/UserScript==
...
function getQueryVariable(variable) {
  ...
  for (let varPair of vars) {
    ...
    if (key === variable) {
      // 先将 URL 中代表空格的 '+' 替换回真正的空格，再进行解码
      let decodedValue = value.replace(/\+/g, '%20');
...
function getKeywords() {
  ...
  if (hostname === 'www.douyin.com' && pathname.startsWith('/search/')) {
    ...
    if (match && match[1]) {
      rawKeywords = decodeURIComponent(match[1]);
    }
  } else {
    for (let mapping of urlMapping) {
      if (mapping.name === "Baidu" && url.includes('www.baidu.com')) {
        const params = new URL(url).searchParams;
        if (params.has('wd')) {
          rawKeywords = params.get('wd');
        } else if (url.includes('word=')) {
          let wd = url.match(/word=([^&]*)/)[1];
          rawKeywords = decodeURIComponent(wd.replace(/\+/g, '%20'));
        }
        break;
      } else if (mapping.testUrl.test(url)) {
        rawKeywords = getQueryVariable(mapping.keyName) || "";
        break;
      }
    }
  }

  // 统一清洗：确保所有搜索引擎提取出来的“+”如果代表空格，都被正确还原为空格
  return rawKeywords;
}
...
function setupSearchLinks(keywords) {
  ...
  urlMapping.forEach(({ name, searchUrl }) => {
    ...
    // 使用 encodeURIComponent 完美地将空格转为 %20
